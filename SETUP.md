...
| `targets-panel.js` | The panel. Paste into `index.html`. |
| `targets-panel.css` | Its styles. Paste into `index.html`. |
| `manifest.json` | Home screen install. |
| `sw.js` | Offline support. |

`index.html` is not here — it's your existing file, with three paste-ins below.
...
1. Create a GitHub repo. Put your existing tool at the root as `index.html`,
   and add every file from this folder alongside it.
...
## Step 2 — Three edits to index.html

**A. Styles.** Paste the whole of `targets-panel.css` inside the existing
`<style>` block, just before `</style>`.

**B. The panel.** In the `<script>` block, delete these two functions:

- `renderRepOpps()` — starts around line 2876
- `renderOverview()` — the one-line shim just after it

Paste the whole of `targets-panel.js` in their place. It defines replacements
for both, so the tab button needs no change.

`toggleRepOppRow()` becomes unused. Leaving it is harmless; delete it if you
want the file tidy.

**C. Head tags.** Just before `</head>`:

```html
<link rel="manifest" href="/manifest.json">
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
<meta name="apple-mobile-web-app-title" content="AGBR">
...
Create `/icons/` with three PNGs exported from the MSS mark:

- `icon-192.png` — 192×192
- `icon-512.png` — 512×512
- `icon-512-maskable.png` — 512×512, logo at ~80% with padding so iOS and
  Android can crop it without clipping
